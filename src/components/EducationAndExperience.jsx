...
function EducationAndExperience() {
    ...
    const renderIcon = (icon) => {
      switch (icon) {
        case 'ibm':
          return <SiIbm className="text-blue-500 ml-2 text-5xl" />;
        case 'infosys':
          return <SiInfosys className="text-blue-500 ml-2 text-5xl" />;
        case 'hackerrank':
          return <SiHackerrank className="text-green-500 text-5xl" />;
        default:
          return null;
      }
    };
  ...
  return (
<> 
    <div className="text-center text-5xl font-bold mb-5 max-[500px]:text-3xl max-[500px]:mb-5"id="ec"> Education And Certificates</div>
    <section
      ...
      <div className="w-full md:w-[80%]">
        ...
        <div
          ...
          <div className="hs-carousel relative overflow-hidden w-full h-48 bg-white dark:bg-gray-800 rounded-lg">
            <div className="hs-carousel-body absolute top-0 bottom-0 start-0 flex flex-nowrap transition-transform duration-700 opacity-0" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
              {/* =========== CERTIFICATES LIST =========== */}
              {user_info.certificates.map((cert, index) => {
                return (
                  <div className="hs-carousel-slide flex-shrink-0 w-full h-full" key={index}>
                    {renderIcon(cert.icon)}

                    <div className="flex py-14 px-8 justify-start items-start h-full bg-gray-100  dark:bg-neutral-900 sm:px-8 md:px-12 lg:px-16 xl:px-24 ">
                      <div className="hs-tooltip [--placement:bottom] inline-block">
                        ...
                        >
                          {/* =========== CERTIFICATE NAME =========== */}
                          <p className="transition duration-700 font-bold">
                            {cert.title}
                          </p>
          ...
          <div className="hs-carousel-pagination flex justify-center absolute bottom-3 start-0 end-0 space-x-2">
            {user_info.certificates.map((cert,index) => {
              return (
                <span
                className={`hs-carousel-active:bg-red-700 hs-carousel-active:border-red-700 size-3 border border-gray-400 rounded-full cursor-pointer dark:border-neutral-600 dark:hs-carousel-active:bg-red-500 dark:hs-carousel-active:border-red-500 ${currentIndex === index ? 'bg-red-700 border-red-700 dark:bg-red-500 dark:border-red-500' : ''}`}
